...
<%-- 검색창 --%>
<%--action을 안쓰면 현재 주소가 action이 됨. get/post를 섞어 쓰면 위험하다 --%>
<form>
	<fieldset>
		<legend>검색창</legend>
		...
		포인트 <input type="number" name="minPoint" value="${vo.minPoint}"> ~ 
				<input type="number" name="maxPoint" value="${vo.maxPoint}"> <br><br>
		가입일 <input type="date" name="memberJoin" value="${vo.memberJoin}"> <br><br>
		최종 로그인 일시 <input type="date" name="minLoginDate" value="${vo.minLoginDate}"> ~ 
		...
		<select name="orderList">
			...
			<option value="member_point desc" <c:if test="${vo.orderList.get(0) == 'member_point desc'}">selected</c:if>>보유 포인트순</option>
			<option value="member_join desc" <c:if test="${vo.orderList.get(0) == 'member_join desc'}">selected</c:if>>최근 가입순</option>
			<option value="member_login desc" <c:if test="${vo.orderList.get(0) == 'member_login desc'}">selected</c:if>>최근 로그인순</option>
		</select>
		<br><br>
		...
		<select name="orderList">
			...
			<option value="member_point desc" <c:if test="${vo.orderList.get(1) == 'member_point desc'}">selected</c:if>>보유 포인트순</option>
			<option value="member_join desc" <c:if test="${vo.orderList.get(1) == 'member_join desc'}">selected</c:if>>최근 가입순</option>
			<option value="member_login desc" <c:if test="${vo.orderList.get(1) == 'member_login desc'}">selected</c:if>>최근 로그인순</option>
		</select>
		<br><br>
		...
		<select name="orderList">
			...
			<option value="member_point desc" <c:if test="${vo.orderList.get(2) == 'member_point desc'}">selected</c:if>>보유 포인트순</option>
			<option value="member_join desc" <c:if test="${vo.orderList.get(2) == 'member_join desc'}">selected</c:if>>최근 가입순</option>
			<option value="member_login desc" <c:if test="${vo.orderList.get(2) == 'member_login desc'}">selected</c:if>>최근 로그인순</option>
		</select>
		<br><br>
