...
//서버에 매우 부담을 주는 위험한 기능이어서 기본적으로 금지가 되어있음
//mail 메소드가 존재하는 파일 위에 @EnableScheduling 을 추가하여 허용 처리
@Service
public class ScheduleService {
	...
	//메소드가 자동으로 실행되도록 @Scheduled와 옵션을 추가
//	@Scheduled(fixedRate = 1000L)
//	@Scheduled(cron = "* * * * * *") //*초 *분 *시 *일 *월 *요일 (연도)
//	@Scheduled(cron = "*/2 * * * * *")// 2초마다(*/2) *분 *시 *일 *월 *요일
	
	//(Q) 2분마다 실행되게 하려면?
//	@Scheduled(cron = "0 */2 * * * *") //2분마다 0초에 실행
//	@Scheduled(cron = "0-10 */2 * * * *") //2분마다 0~10초에 실행
//	@Scheduled(cron = "0,10 */2 * * * *") //2분마다 0초와 10초에 실행
	
	//(Q) 출근시간(9시)에 한 번, 퇴근시간(18시)에 한 번 실행
...
//	@Scheduled(cron = "0 0 15 25 * ?") //물음표는 무관(any)이라는 뜻
	...
	//(Q) 매월 넷째 주 수요일 오후 5시에 실행
//	@Scheduled(cron = "0 0 17 ? * 3#4") //넷째 주(#4) 수요일(3)
//	@Scheduled(cron = "0 0 17 ? * wed#4") //넷째 주(#4) 수요일(wed)
