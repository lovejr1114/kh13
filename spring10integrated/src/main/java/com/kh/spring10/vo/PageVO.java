...
public class PageVO {
	private String column, keyword; //검색 항목, 검색어
	private int page=1; //현재 페이지
	private int size=10; //한 페이지에 보여줄 게시글 개수
	private int count; //전체 글 수 (전체 개수)
	...
	//물음표 뒤에 붙는 데이터 중 공통된 부분 (size, column, keyword)에 대한 문자열 
	public String getQueryString() {
		return "size="+size+"&column="+column+"&keyword="+getKeyword();
	}
