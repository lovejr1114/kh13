...
/*
   ...
   - .check-all 은 전체 선택 체크박스
    - .check-item은 개별 항목 체크박스
    - jquery에서는 태그의 속성을 .props() 명령으로 제어 또는 확인
 */
     $(function(){
       ...
       // $(".check-all").on("input",function(){});
        ...
        // var checked = this.checked; //JS
            ...
            $(".check-item").prop("checked",checked); //개별항목 체크 변경
            $(".check-all").prop("checked",checked); //전체항목 체크 변경
        });
        
        $(".check-item").change(function(){
            // 총 ?개 중 ?개가 선택 되었는지를 파악하여 처리
            var countAll = $(".check-item").length; //개수 확인하고싶으면 무조건 length
