...
//		votes.put("라이츄", 3);
		...
		while(true) {
			...
			if(votes.containsKey(user)) {//투표 이력이 있으면
//				System.out.println("기록 유");
//				Integer count = votes.get(user); //null이 나올 수 있으면 (조건에 key가 있으니 null이 나올 일 없다)
				int count = votes.get(user); //null이 나올 수 없으면
				//기존 득표수를 꺼내어
